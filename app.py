...
from utils.data_loader import (
    ...
    CRUSTAL_LAYERS, FDSN_PROVIDERS, fetch_fdsn_waveform,
)
...
STRINGS = {
    ...
    "src_fdsn":         {"en": "INGV / IRIS Live", "it": "INGV / IRIS Live", "fr": "INGV / IRIS Live", "es": "INGV / IRIS Live", "de": "INGV / IRIS Live"},
    "fdsn_provider":    {"en": "Provider", "it": "Provider", "fr": "Fournisseur", "es": "Proveedor", "de": "Anbieter"},
    "fdsn_network":     {"en": "Network code", "it": "Codice rete", "fr": "Code réseau", "es": "Código de red", "de": "Netzwerkkürzel"},
    "fdsn_station":     {"en": "Station code", "it": "Codice stazione", "fr": "Code station", "es": "Código estación", "de": "Stationskürzel"},
    "fdsn_location":    {"en": "Location (or --)", "it": "Location (o --)", "fr": "Location (ou --)", "es": "Location (o --)", "de": "Location (oder --)"},
    "fdsn_channel":     {"en": "Channel", "it": "Canale", "fr": "Canal", "es": "Canal", "de": "Kanal"},
    "fdsn_start":       {"en": "Start (UTC)", "it": "Inizio (UTC)", "fr": "Début (UTC)", "es": "Inicio (UTC)", "de": "Start (UTC)"},
    "fdsn_end":         {"en": "End (UTC)", "it": "Fine (UTC)", "fr": "Fin (UTC)", "es": "Fin (UTC)", "de": "Ende (UTC)"},
    "fdsn_fetch":       {"en": "⬇ Fetch waveform", "it": "⬇ Scarica forma d'onda", "fr": "⬇ Récupérer la forme d'onde", "es": "⬇ Obtener forma de onda", "de": "⬇ Wellenform abrufen"},
    "fdsn_hint":        {
        "en": "Connects directly to <b>INGV</b>, <b>IRIS</b>, <b>ORFEUS</b> and other FDSN data centres. No file download needed — the waveform streams directly into SeismicLens.",
        "it": "Si connette direttamente a <b>INGV</b>, <b>IRIS</b>, <b>ORFEUS</b> e altri data centre FDSN. Nessun file da scaricare — la forma d'onda arriva direttamente in SeismicLens.",
        "fr": "Se connecte directement à <b>INGV</b>, <b>IRIS</b>, <b>ORFEUS</b> et d'autres centres de données FDSN. Pas de téléchargement — la forme d'onde arrive directement dans SeismicLens.",
        "es": "Se conecta directamente a <b>INGV</b>, <b>IRIS</b>, <b>ORFEUS</b> y otros centros de datos FDSN. Sin descargar archivos — la forma de onda llega directamente a SeismicLens.",
        "de": "Verbindet direkt mit <b>INGV</b>, <b>IRIS</b>, <b>ORFEUS</b> und anderen FDSN-Datenzentren. Kein Datei-Download nötig — die Wellenform streamt direkt in SeismicLens.",
    },
    "fdsn_example":     {
        "en": "Example: INGV · IV · ACER · 00 · HHZ · 2016-08-24T01:36:00 → 01:40:00 (Amatrice Mw6.2)",
        "it": "Esempio: INGV · IV · ACER · 00 · HHZ · 2016-08-24T01:36:00 → 01:40:00 (Amatrice Mw6.2)",
        "fr": "Exemple : INGV · IV · ACER · 00 · HHZ · 2016-08-24T01:36:00 → 01:40:00 (Amatrice Mw6.2)",
        "es": "Ejemplo: INGV · IV · ACER · 00 · HHZ · 2016-08-24T01:36:00 → 01:40:00 (Amatrice Mw6.2)",
        "de": "Beispiel: INGV · IV · ACER · 00 · HHZ · 2016-08-24T01:36:00 → 01:40:00 (Amatrice Mw6.2)",
    },
    "fdsn_fetching":    {"en": "Fetching data from", "it": "Recupero dati da", "fr": "Récupération depuis", "es": "Obteniendo datos de", "de": "Daten abrufen von"},
    "fdsn_ok":          {"en": "Waveform loaded from", "it": "Forma d'onda caricata da", "fr": "Forme d'onde chargée depuis", "es": "Forma de onda cargada de", "de": "Wellenform geladen von"},
    "fdsn_error":       {"en": "FDSN fetch error", "it": "Errore FDSN", "fr": "Erreur FDSN", "es": "Error FDSN", "de": "FDSN-Fehler"},
    "mseed_hint":       {
...
with st.sidebar:
    ...
    data_source = src_options.index(data_source_label)  # 0=synthetic, 1=mseed, 2=csv, 3=fdsn

    uploaded_file = None
    fdsn_params   = None   # will hold dict of FDSN query params when source==3
    ...
    if data_source == 1:
        ...
        st.markdown(f"<div class='info-box' style='font-size:11px;'>{T('fdsn_hint', lang)}</div>",
                    unsafe_allow_html=True)
        fdsn_provider = st.selectbox(T("fdsn_provider", lang), list(FDSN_PROVIDERS.keys()))
        fdsn_network  = st.text_input(T("fdsn_network", lang),  value="IV",  max_chars=8)
        fdsn_station  = st.text_input(T("fdsn_station", lang),  value="ACER", max_chars=8)
        fdsn_location = st.text_input(T("fdsn_location", lang), value="00",  max_chars=4)
        fdsn_channel  = st.text_input(T("fdsn_channel", lang),  value="HHZ", max_chars=4)
        fdsn_start    = st.text_input(T("fdsn_start", lang),    value="2016-08-24T01:36:00")
        fdsn_end      = st.text_input(T("fdsn_end", lang),      value="2016-08-24T01:40:00")
        st.caption(f"💡 {T('fdsn_example', lang)}")
        fdsn_fetch_btn = st.button(T("fdsn_fetch", lang), use_container_width=True)
        if fdsn_fetch_btn:
            fdsn_params = dict(
                provider_name=fdsn_provider,
                network=fdsn_network,
                station=fdsn_station,
                location=fdsn_location,
                channel=fdsn_channel,
                starttime=fdsn_start,
                endtime=fdsn_end,
            )
            st.session_state["fdsn_params"] = fdsn_params
            # Invalida segnale cached → verrà ri-scaricato al prossimo render
            st.session_state.pop("fdsn_signal", None)
        # Persist last successful fetch across rerenders
        if fdsn_params is None and "fdsn_params" in st.session_state:
            fdsn_params = st.session_state["fdsn_params"]
        # Mostra info traccia cached nella sidebar
        if "fdsn_signal" in st.session_state:
            _cm = st.session_state["fdsn_signal"]["metadata"]
            st.markdown(
                f"<div class='success-box' style='font-size:11px;'>✅ "
                f"{_cm.get('network','')}.{_cm.get('station','')}."
                f"{_cm.get('location','')}.{_cm.get('channel','')} &nbsp;·&nbsp; "
                f"{_cm.get('npts','')} campioni @ {_cm.get('sampling_rate_hz','')} Hz</div>",
                unsafe_allow_html=True,
            )
...
if data_source == 0:
    ...
    if "fdsn_signal" in st.session_state:
        ...
        fs = st.session_state["fdsn_signal"]["fs"]
        metadata = st.session_state["fdsn_signal"]["metadata"]
    else:
        with st.spinner(f"{T('fdsn_fetching', lang)} {_prov} …"):
            try:
                signal_raw, fs, metadata = fetch_fdsn_waveform(**fdsn_params)
                st.success(f"✅ {T('fdsn_ok', lang)} **{_prov}** — "
                           f"{metadata['network']}.{metadata['station']}."
                           f"{metadata['location']}.{metadata['channel']}  "
                           f"({metadata['npts']} samples @ {fs} Hz)")
                # Salva in session_state per evitare HTTP request ripetute
                st.session_state["fdsn_signal"] = {
                    "signal_raw": signal_raw,
                    "fs": fs,
                    "metadata": metadata,
                }
            except Exception as e:
                st.error(f"{T('fdsn_error', lang)}: {e}")
